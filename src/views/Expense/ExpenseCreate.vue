<template>
	<div
		class="modal fade"
		id="expenseCreateModal"
		tabindex="-1"
		aria-labelledby="expenseCreateModalLabel"
		aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h1 class="modal-title fs-5" id="expenseCreateModalLabel">
						Expense Create
					</h1>
					<button
						type="button"
				...
				<template v-if="!loading">
					<div class="modal-body">
						<form @submit.prevent>
							<div
								class="d-flex gap-5 justify-content-center mb-3 border rounded p-3">
							...
							<div class="mb-3" v-if="paymentType === 'Bank'">
								<label for="bankAccounts" class="form-label"
									>Select Bank Account</label
								>
								<select
									id="bankAccounts"
									v-model="payload.selectedBankAccount"
									class="form-select"
									aria-label="select bank account">
									<option disabled selected>Select Bank Account</option>
									<option v-for="each in bankAccounts" :value="each">
										{{ each.subHeader }}
									</option>
								</select>
							</div>
							<div class="mb-3" v-if="paymentType === 'Cash'">
								<label for="cashAccounts" class="form-label"
									>Select Cash Account</label
								>
								<select
									id="cashAccounts"
									v-model="payload.selectedCashAccount"
									class="form-select"
									aria-label="select cash account">
									<option disabled selected>Select Cash Account</option>
									<option v-for="each in cashAccounts" :value="each">
										{{ each.name }}
									</option>
								</select>
							</div>
							<div class="mb-3">
								<label for="expenseAccount" class="form-label"
									>Expense Account</label
								>
								<VueMultiselect
									:model-value="selectedExpenseAccount"
									@update:model-value="updateSelectedExpenseAccount"
									...
									:option-height="31"
									track-by="_id"
									:custom-label="customExpenseAccountListLabel"
							...
							<div class="row mb-3">
								<div class="col">
									<div class="mb-3">
										...
										>
										<input
											@change="calculateFinalCurrency"
											v-model="payload.initialAmount"
											type="number"
											class="form-control"
											id="initialAmount" />
									</div>
								...
								<div class="col">
									<div class="mb-3">
										<label for="initialCurrency" class="form-label"
											>Initial Currency</label
										>
										<select
											@change="calculateFinalCurrency"
											v-model="payload.selectedInitialCurrency"
											class="form-select"
											aria-label="Initial currency">
											<option disabled selected>Select Initial Currency</option>
											<option v-for="each in currencyList" :value="each">
												{{ each.name }}
											</option>
										</select>
									</div>
							...
							<div class="row mb-3">
								<div class="col">
									...
									<input
										@change="calculateFinalCurrency"
										v-model="payload.finalAmount"
										type="number"
										class="form-control"
										id="finalAmount" />
								</div>
								<div class="col">
									<label for="finalCurrency" class="form-label"
										>Final Currency</label
									>
									<select
										@change="calculateFinalCurrency"
										v-model="payload.selectedFinalCurrency"
										class="form-select"
...
const calculateFinalCurrency = () => {
	if (
		payload.value.selectedInitialCurrency.code === "USD" &&
		payload.value.selectedFinalCurrency.code === "MMK"
	) {
		payload.value.finalAmount = Math.round(
			payload.value.initialAmount *
				Number(payload.value.selectedInitialCurrency.exchangeRate)
		)
		console.log(payload.value.finalAmount)
	} else if (
		payload.value.selectedInitialCurrency.code === "MMK" &&
		payload.value.selectedFinalCurrency.code === "USD"
	) {
		payload.value.finalAmount = Math.round(
			payload.value.initialAmount /
				Number(payload.value.selectedFinalCurrency.exchangeRate)
		)
	} else {
...
const prepareForm = () => {
	let data = {
		...
		date: payload.value.date
			? dateForParams(payload.value.date)
			: dateForParams(),
...
const fetchCurrencies = async () => {
	try {
		...
		payload.value.selectedInitialCurrency = currencyList.value.filter(
			each => each.code === "MMK"
		)[0]
		payload.value.selectedFinalCurrency = currencyList.value.filter(
			each => each.code === "MMK"
		)[0]
	} catch (error) {
		console.log(error)
